$(function() {
	...
	setTimeout(function() {
		...
		if ($(".product-attribute-selection :first").length) {
			...
			if ($(".add-to-cart-zone .alert-danger").is(":visible")) {
	...
	body.on("change", ".product-attribute-selection", function(e) {
		...
		$.ajax({
			...
			if (!$.isEmptyObject(data.variant)) {
				body
					.find(".single-product-description")
					.data("variantid", parseInt(data.variant.var_id));
			}
			...
			$.each(attributes, function(key, value) {
				...
				if (
					$.inArray(parseInt($("#prd-attr-" + key).val()), attrOptions) == -1
				) {
			...
			if (
				...
				if (varStock != 0) {
					...
					$("form#add-to-cart-form input[name='m_variant_country']").val(
						data.countryId
					);
					$("form#add-to-cart-form input[name='m_variant_quantity']").data(
						"maxvalue",
						varStock
					);
				} else {
					$(".add-to-cart-zone .alert-danger").removeClass("hidden");
					$(".add-to-cart-zone .alert-danger").html(
