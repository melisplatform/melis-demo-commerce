   ...
   ------------------------------ */
	...
	$(".cart-plus-minus-box").on("change", function() {
		...
		if (discount) {
			...
			parent
				.find(".item-discount")
				.text(priceCurrency + "" + discount.toFixed(2));
		}

		parent
			.find(".total-price strong")
			.text(priceCurrency + "" + tmpTotal.toFixed(2));
	});
});
...
function setAddToCartState(disable) {
	...
	if (disable) {
		...
		$(".add-to-cart-quantity-zone .cart-plus-minus-box").addClass(
			"disable-cart-plus-minus-box"
		);
		$(".add-to-cart-quantity-zone .qtybutton").addClass("disable-qtybutton");
		$(".add-to-cart-quantity-zone .btn-add-cart").addClass(
			"disable-btn-add-cart"
		);
	} else {
		$(".add-to-cart-quantity-zone .cart-plus-minus-box").attr(
			"readonly",
			false
		);
		$(".add-to-cart-quantity-zone .cart-plus-minus-box").removeClass(
			"disable-cart-plus-minus-box"
		);
		$(".add-to-cart-quantity-zone .qtybutton").removeClass("disable-qtybutton");
		$(".add-to-cart-quantity-zone .btn-add-cart").removeClass(
			"disable-btn-add-cart"
		);
	}
}
